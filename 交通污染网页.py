...
def predict():
    try:
        ...
        result_text = f"预测类别：{category_mapping[predicted_class]}<br>预测概率：{probability_str}<br>{advice}"
        ...
        # 设置图表属性
        plt.title(f'预测类型为{predicted_class_name}时的特征贡献度瀑布图', fontsize=30, fontproperties=font_prop)
